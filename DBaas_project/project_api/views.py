...
class ClusterViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if provider_name == 'Kubernetes' and cluster_type == 'Standalone':
            ...
            trigger_infra(base_url, project_id, headers, 'infra-db')
        ...
        if cluster_type1 == True:
            ...
            response = trigger_single(base_url, project_id, headers, 'single-node-db')
    ...
    def destroy_cluster(self, request):
        """
        Trigger the destroy pipeline for a cluster.
        """
        print("destroy_cluster function run")
        cluster_name = request.data.get('cluster')
        print(cluster_name)
        

        # You may need to adapt these lines based on your specific GitLab setup
        project_id = "1"  # Replace with your actual GitLab project ID
        private_token = "glpat-QnYftX2oXsc9N5xSxG4n"  # Replace with your actual GitLab private token
        base_url = "http://gitlab-ce.os3.com/api/v4/"  # Replace with your actual GitLab base URL

        headers = {"PRIVATE-TOKEN": private_token}
        pipeline_name = 'destroy'  # Replace with the actual name of your destroy pipeline

        response = trigger_single(base_url, project_id, headers, pipeline_name)
        
        if response == 200:
            # Assuming your destroy pipeline runs asynchronously, you may want to handle success accordingly
